...
class SEQDIFF_sampler:
    ...
    def potential_init(self):
        ...
        for p,s in zip(potentials, potential_scale):
            ...
            pot = POTENTIALS[p](self.args, self.features, s, self.DEVICE)
            self.potential_list.append(pot)
            if hasattr(pot, 'base_cos_sim'):
                self.base_cos_sim = pot.base_cos_sim
                # initialize best-cos metrics
                self.best_cos_sim = -1
                self.best_cos_seq = None
                self.best_cos_xyz = None
    ...
    def save_step(self):
        ...
        mean_plddt = float(self.features['pred_lddt'].mean().item())
        self.trajectory[f'plddt_step{self.t.item()}'] = mean_plddt
    ...
    def apply_potentials(self):
        ...
        if self.args['potential_scale_schedule']:
            mult = 1.0 - (self.t.item() / float(self.max_t))
            print(f"Potential Scale Multiplier: {mult:.4f}")
            self.features['seq_out'] += mult * (grads/len(self.potential_list))
        else:
    ...
    def generate_sample(self):
        ...
        # log the baseline cos–sim
        if hasattr(self, 'base_cos_sim'):
            self.trajectory['baseline_cos_sim'] = self.base_cos_sim
        ...
        for j in range(self.max_t):
            ...
            if self.use_potentials:
                ...
                for p in self.potential_list:
                    if hasattr(p, 'current_cos_sim'):
                        # stash into trajectory
                        step = self.t.item()
                        self.trajectory[f'cos_sim_step{step}'] = p.current_cos_sim
                        self.trajectory[f'seq_dist_step{step}'] = p.current_seq_distance
                        if p.current_cos_sim > self.best_cos_sim:
                            self.best_cos_sim = p.current_cos_sim
                            self.best_cos_seq = torch.clone(self.features['seq'])
                            self.best_cos_xyz = torch.clone(self.features['xyz'])
            ...
            print("    TIMESTEP [%02d/%02d]   |   current PLDDT: %.4f   <<  >>   best PLDDT: %.4f   ||   best COS_SIM: %.4f" % (
                self.t + 1, self.args['T'], self.features['pred_lddt'].mean().item(),
                self.features['best_pred_lddt'].mean().item(), self.best_cos_sim))
    ...
    def save_outputs(self):
        ...
        if self.use_potentials and hasattr(self, 'best_cos_seq') and self.best_cos_seq is not None:
            cos_fname = self.out_prefix + '_best_cos.pdb'
            write_pdb(cos_fname,
                      self.best_cos_seq.type(torch.int64).squeeze(),
                      self.best_cos_xyz.squeeze(),
                      Bfacts=self.features['pred_lddt'].squeeze(),
                      chains=chain_ids)
